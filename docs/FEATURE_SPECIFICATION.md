...
The existing `job_freq` and `merchant_freq` features are inherited from
the Data Science preprocessing pipeline and are outside the scope of the
MLOps historical feature pipeline.

These features are based on dataset-level frequency counts and are not
treated as point-in-time historical features.

Therefore:

- They are not generated by `src/features/transformations.py`.
- They are not included in the historical rolling feature leakage tests.
- Their leakage behavior depends on how the Data Science preprocessing
  pipeline constructs them.

If these features are used in production or future model training
pipelines, their frequency counts should be computed using training data
only, or replaced with point-in-time frequency features.

The MLOps feature pipeline currently focuses on point-in-time-safe
card-level and merchant-level rolling features.
